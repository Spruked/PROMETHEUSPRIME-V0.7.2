class WhisperingArchive:
    ...
    def log_with_emotion(self, message, emotion=None, glyph=None):
        self.record(message, emotion=emotion, glyph=glyph)

    def log(self, message, actor="Unknown"):
        entry = {
            "actor": actor,
            "message": message
        }
        self.logs.append(entry)
        print(f"[Archive] Log saved: {entry}")
